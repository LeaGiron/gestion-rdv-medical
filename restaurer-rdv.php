...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["rdv_id"])) {
    ...
    try {
        // Mise à jour du statut du rendez-vous en "Confirmé"
        $stmt = $pdo->prepare("UPDATE rendez_vous SET rdv_statut_rendez_vous = 'Confirmé' WHERE rdv_id_rendez_vous = ?");
        $stmt->execute([$rdv_id]);
        ...
        $_SESSION['confirmation_message'] = "Le rendez-vous avec " . htmlspecialchars($rdv['patient_nom']) . " " . htmlspecialchars($rdv['patient_prenom']) . " a été restauré et confirmé.";
